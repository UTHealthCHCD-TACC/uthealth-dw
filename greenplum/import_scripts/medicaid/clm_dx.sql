...
LOCATION ( 
'gpfdist://greenplum01:8081/uthealth/medicaid/*/CLM_DX_*.csv#transform=add_parentname_filename_comma'
)
...
-- 318 secs
update medicaid.clm_dx set year=date_part('year', FST_DT) where year=0;
...
-- Verify
select count(*), min(year), max(year), count(distinct year) from medicaid.clm_dx;
