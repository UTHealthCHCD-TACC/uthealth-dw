...
create temp table tmp_trvc_srvcs as
	select 'trvc' as dt_src, trunc(coalesce(s.msclmid, s.caseid),0) claim_no, trunc(s.enrolid,0) enrolid, 
	trunc(s."year",0) s_year, trunc(s.msclmid,0) msclmid ,trunc(s.seqnum,0) seqnum, 
	m.uth_member_id, get_my_from_date(s.admdate) mon_year, s.proc1, substring(s.procmod, 1,1) as procmod1, s.stdplac,
	...
	from truven.ccaes s inner join tmp_uth_members m
...
from tmp_trvc_srvcs t inner join tmp_uth_claims u 
