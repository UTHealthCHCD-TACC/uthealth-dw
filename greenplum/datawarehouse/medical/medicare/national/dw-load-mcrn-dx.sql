delete from data_warehouse.claim_diag where data_source = 'mcrn';
...
insert into data_warehouse.claim_diag (data_source, year, uth_member_id, uth_claim_id, claim_sequence_number
									  ,date, diag_cd, diag_position, data_year ) 									 									  
...
select count(*), left(m.tos_cd,(position('.' in m.tos_cd)-1)), year  
from optum_dod.medical m 
group by left(m.tos_cd,(position('.' in m.tos_cd)-1)), year  

select distinct service_type from reference_tables.ref_optum_cost_factor rocf 
