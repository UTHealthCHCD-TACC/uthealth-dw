...
-- Row count:
select 'total row count in data_warehouse.dim_uth_rx_claim_id table' as message, count(*) from data_warehouse.dim_uth_rx_claim_id where data_source = 'iqva'; -- CNT: # 3233703887

--````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````

-- Compare total claimno counts (should equal the row count, 3233703887):
select 'total claim count (uth_rx_claim_id) in data_warehouse.dim_uth_rx_claim_id table' as message, count(distinct uth_rx_claim_id) from data_warehouse.dim_uth_rx_claim_id where data_source = 'iqva'; -- CNT: # 3233703887
select 'total claim count (rx_claim_id_src) in data_warehouse.dim_uth_rx_claim_id table' as message, count(distinct rx_claim_id_src) from data_warehouse.dim_uth_rx_claim_id where data_source = 'iqva'; -- CNT: # 3233703887

-- total claim count from iqvia raw claims table (should be 3233703887):
...
where new_rectype = 'P'; -- CNT: # 3233703887

--````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````

-- Compare total patient counts:
select 'total pat count (uth_member_id) in data_warehouse.dim_uth_rx_claim_id' as message, count(distinct uth_member_id) from data_warehouse.dim_uth_rx_claim_id where data_source = 'iqva'; -- CNT: # 75877046
select 'total pat count (member_id_src) in data_warehouse.dim_uth_rx_claim_id' as message, count(distinct member_id_src) from data_warehouse.dim_uth_rx_claim_id where data_source = 'iqva'; -- CNT: # 75877046

-- total patient count from iqvia raw claims table (should be 75877046):
select 'total pat count in raw iqvia claims table' as message, count(distinct pat_id) 
from dev.sa_iqvia_derv_claimno_new_all_yr 
where new_rectype = 'P'; -- CNT: # 75877046
