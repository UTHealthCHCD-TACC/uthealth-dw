...
select distinct data_source from data_warehouse.t_member_enrollment_monthly_rowandparts
/* As of 02oct2019: mdcr, trvm, trvc, optz, optd */

/* create the partititioned monthly enrollment table */
create table data_warehouse.t_member_enrollment_monthly_rowandparts (
    --ID columns
	id bigserial,
	data_source char(4), 
	month_year_id int4,
...
insert into data_warehouse.t_member_enrollment_monthly_rowandparts
select * from data_warehouse.member_enrollment_monthly where data_source <> 'mdcr'

analyze data_warehouse.t_member_enrollment_monthly_rowandparts

select count(id) from data_warehouse.t_member_enrollment_monthly_rowandparts

/* check to make sure that the correct partitions and number of partitions were created */
select * from pg_catalog.pg_partitions where tablename='t_member_enrollment_monthly_rowandparts'

/* check to make sure that the data is getting evenly distributed across all the segments */
select gp_segment_id, count(id)
from data_warehouse.t_member_enrollment_monthly_rowandparts
