...
#this is the list of tables to look in and count for icd_proc
...
```
## 1.4: Count distinct enrolid-msclmid combinations (has dx only, enrolid does not have to exist in 'a' tables)
```{r}
...
## 1.5: Count distinct enrolid-msclmid combinations with procs
```{r}
...
# sheet 3: distinct enrolid || msclmids ONLY for rows with dx, enrolid does not have to exist in 'a' tables
current_sheet = 3; current_df = truv_dx_tbl2
writeData(wb, sheet = current_sheet,
...
# sheet 4: distinct enrolid || msclmids ONLY for rows with proc codes, enrolid does not have to exist in 'a' tables
current_sheet = 4; current_df = truv_proc_tbl
writeData(wb, sheet = current_sheet,
...
# sheet 5: distinct enrolid || ndcnum || svcdate from 'd' tables, where enrolid is not null
current_sheet = 5; current_df = truv_rx_tbl
writeData(wb, sheet = current_sheet,
