...
getNumObservations <- function(year){
  ...
  setwd(paste0('V:/Data Dictionary/Data Quality Reports/CCAE-MDCR/',year))
  
  # Retrieves and stores MDCR and CCAE file names from a directory for a given year
  file_names <- list.files(paste0('V:/Data Dictionary/Data Quality Reports/CCAE-MDCR/',year), pattern = c('mdcr|ccae'))
  
  # Loops through file_names to pull the number of observations and report version, and add the value to numObservationsDF (along with corresponding table 
  # name and year) for each MDCR and CCAE .txt file in the working directory
  for (i in 1:length(file_names)){
    ...
    # Finds and retrieves line of the first occurrence of 'Version' within the .txt file
    reportVersionTxtLine <- readTxt[grepl('Version', readTxt)][1]
    ...
    # Formats and cleans up the line read from the .txt file (reportVersionTxtLine):
    # sub() is used to select everything after 'Version' in the string
    # str_replace_all() is used to remove any dashes, empty space, or commas that may be in the string
    reportVersion <- str_replace_all(sub('.+Version(.+)', '\\1', reportVersionTxtLine), c('-' = '', ' ' = '', ',' = ''))
    ...
    numObservationsDF[nrow(numObservationsDF) +1,] <- c(year, substring(file_names[i], 1,5), numObservations, reportVersion)
...
rowsToBeUpdated <- dbGetQuery(tac,"select * from qa_reporting.truven_counts where reported_row_count is null or row_count_difference  != 0;")
