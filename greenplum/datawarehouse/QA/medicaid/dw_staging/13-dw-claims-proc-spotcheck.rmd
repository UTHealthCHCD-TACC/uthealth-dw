...
# PURPOSE
This R markdown script spot checks for the existence of proc codes in dw_staging
...
09/13/23 Modifications:
  ...
  * In section 4.2, a Notes section was added to the xlsx output. 
...
from dev.xz_dwqa_temp2 a left join dw_staging.mcd_claim_icd_proc b on a.icn = b.claim_id_src and a.proc = b.proc_cd;
...
writeData(wb, sheet = 1, "Mismatches found in claim_icd_proc tables between SPC and dw_staging", startCol=1, startRow=1)
...
# Add Notes
nextrow = length(output$fy) + 8
writeData(wb, sheet = 1, "Notes:", startCol=1, startRow= nextrow)
nextrow = nextrow + 1
writeData(wb, sheet = 1, "The large number for 2019's 'fy_mistmatch' is due to Medicaid providing FY2020 claims in the FY2019 table. So, a large number of FY mismatches for 2019 is expected.", startCol=1, startRow=nextrow)
