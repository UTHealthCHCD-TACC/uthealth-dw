---
title: "dwqa-truven-claim-icd-proc-counts"
author: "XRZ"
date: '2023-01-10'
...
here::i_am("06-claim-icd-proc-counts.Rmd")
```
...
if (!file.exists(path)) {
 ...
 print("Missing counts from Truven schema! Go back and run 02-truven-schema-counts.Rmd")
  ...
  truv_proc_in = read.xlsx(path, sheet = 4, startRow = 4, rows=c(4:16),
...
dw_sql2 = "create table dev.xz_dwqa_temp2 as
  select year, member_id_src || claim_id_src as concat from dw_staging.claim_icd_proc
  where year = @year
  distributed by (concat);"
...
truv_proc_in$truv_proc_ccae = as.integer(truv_proc_in$truv_proc_ccae)
truv_proc_in$truv_proc_mdcr = as.integer(truv_proc_in$truv_proc_mdcr)

# rearrange dw table so that each column is a table_id_src
truv_squish = truv_proc_in %>%
  mutate(truv_concat = truv_proc_ccae + truv_proc_mdcr) %>%
...
wb = createWorkbook()
dir = here("outputs")
path = here("outputs", "06-truven-claim-icd-proc-counting.xlsx")
