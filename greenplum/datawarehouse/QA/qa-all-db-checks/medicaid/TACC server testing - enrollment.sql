...
select b.mem_id as member_id, b.cy as year,
	...
	case when ((a.total_enrolled_months is null and b.enrolled_months is not null) or
		...
		a.total_enrolled_months != b.enrolled_months)
		then 1 else 0 end as em_mismatch,
	case when ((a.gender_cd is null and b.sex is not null) or
		(a.gender_cd is not null and b.sex is null) or 
		a.gender_cd != b.sex)
		then 1 else 0 end as sex_mismatch,
	case when ((c.race_cd_src is null and (b.race is not null and b.race != '6')) or
		((c.race_cd_src is not null and c.race_cd_src != '6') and b.race is null) or 
		c.race_cd_src != b.race)
		then 1 else 0 end as race_mismatch,
	case when ((a.dob_derived is null and b.dob is not null) or
		(a.dob_derived is not null and b.dob is null) or 
		a.dob_derived != b.dob::date) then 1 else 0 end as dob_mismatch,
	case when ((a.zip5 is null and b.zip is not null) or
		(a.zip5 is not null and b.zip is null) or 
		a.zip5 != b.zip) then 1 else 0 end as zip_mismatch,
	case when ((a.plan_type is null and b.mco is not null) or
		(a.plan_type is not null and b.mco is null) or 
		trim(a.plan_type) != trim(b.mco)) then 1 else 0 end as plan_mismatch,
	case when ((a.dual is null and b.smib is not null) or
		(a.dual is not null and b.smib is null) or 
		a.dual::text !=  b.smib) then 1 else 0 end as dual_mismatch,
	case when ((a.htw is null and b.htw is not null) or
		(a.htw is not null and b.htw is null) or 
		a.htw != b.htw) then 1 else 0 end as htw_mismatch,
...
select * from dw_staging.member_enrollment_yearly where member_id_src = '701644512';

701644512	2014
617650720	2019

select case when (null = '1') then 1 else 0 end as test;
