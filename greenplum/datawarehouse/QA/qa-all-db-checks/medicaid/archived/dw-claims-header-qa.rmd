...
# SECTION 1: Counting distinct ICNs

## 1.1 Get distinct ICN counts from headers from data_warehouse
...
select count(distinct claim_id_src)
...
## 1.2 Get distinct ICN counts from headers from TACC medicaid schema
```{sql, connection = tac, output.var = distinct_ICN_det_TACCmcd}
...
```{sql, connection = tac, output.var = distinct_ICN_proc_TACCmcd}
--get distinct ICNs from medicaid schema (TACC) by year
select count(distinct icn)
...
## 1.4 Get distinct ICN counts from headers inner join procs from TACC medicaid schema
```{sql, connection = tac, output.var = distinct_ICN_innerjoin_TACCmcd}
--get distinct ICNs from medicaid schema (TACC) by year
select count(distinct a.icn)
from (select year_fy, icn from medicaid.clm_header
	union select year_fy, derv_enc from medicaid.enc_header) a
	 inner join
...
from (select icn from medicaid.clm_header
	union select derv_enc from medicaid.enc_header
	union select icn from medicaid.htw_clm_header) a
	 inner join
...
```{sql, connection = tac, output.var = total_fix_temp, eval=FALSE}
--This chunk doesn't get run, it's to fix a few things

--get distinct ICNs from medicaid schema (TACC, Total)
select count(*)
from (select icn from medicaid.clm_header 
	union select derv_enc from medicaid.enc_header
	union select icn from medicaid.htw_clm_header) z;
...
select count(distinct a.icn)
from medicaid.htw_clm_header a inner join medicaid.htw_clm_proc b
  on a.icn = b.icn;
...
Let's just count distinct by year (headers, procs, and inner joins of the two) and compare to TACC

```{r}
#declare base queries to modify for getting the count of distinct ICNs each year

#counts it from headers
sql4 = 'select count(*)
from (select icn from medicaid.dbo.clm_header_@yr
	union select derv_enc from medicaid.dbo.enc_header_@yr) z;'
...
#counts the INNER JOIN of headers and procs (center piece of venn diagram)
sql6 = 'select count(*)
from ((select a.icn from medicaid.dbo.clm_header_@yr a
	inner join medicaid.dbo.clm_proc_@yr b on a.icn = b.icn)
	union
	(select c.derv_enc from medicaid.dbo.enc_header_@yr c
...
distinct_icns_by_year_spc = data.frame(matrix(data=NA, nrow=(21-13+4), ncol=3))

colnames = c("from_headers", "from_procs", "inner_join")
...
distinct_icns_by_year_spc[1,1] = dbGetQuery(spc, 'select count(distinct icn) from medicaid.dbo.clm_header_12;')
distinct_icns_by_year_spc[1,2] = dbGetQuery(spc, 'select count(distinct icn) from medicaid.dbo.clm_proc_12;')
distinct_icns_by_year_spc[1,3] = dbGetQuery(spc, 'select count(distinct a.icn) from medicaid.dbo.clm_header_12 a
...
select count (distinct icn) from medicaid.dbo.CLM_HEADER_1819_HTW;

select count (distinct icn) from medicaid.dbo.CLM_PROC_1819_HTW;

select count (distinct a.icn)
from medicaid.dbo.CLM_HEADER_1819_HTW a inner join
medicaid.dbo.CLM_PROC_1819_HTW b on a.icn = b.icn;
```

## 1.8 Get total distinct ICNs from header tables from SPC server
```{sql, connection = spc, output.var = total_distinct_ICN_header_spc}

select count(*)
from (select icn from medicaid.dbo.clm_header_12 union
	select icn from medicaid.dbo.clm_header_13 union
	select icn from medicaid.dbo.clm_header_14 union
	select icn from medicaid.dbo.clm_header_15 union
	select icn from medicaid.dbo.clm_header_16 union
	select icn from medicaid.dbo.clm_header_17 union
	select icn from medicaid.dbo.clm_header_18 union
	select icn from medicaid.dbo.clm_header_19 union
	select icn from medicaid.dbo.CLM_HEADER_1819_HTW union
	select icn from medicaid.dbo.clm_header_20 union
	select icn from medicaid.dbo.clm_header_21 union
	select derv_enc from medicaid.dbo.enc_header_13 union
	select derv_enc from medicaid.dbo.enc_header_14 union
	select derv_enc from medicaid.dbo.enc_header_15 union
	select derv_enc from medicaid.dbo.enc_header_16 union
	select derv_enc from medicaid.dbo.enc_header_17 union
	select derv_enc from medicaid.dbo.enc_header_18 union
	select derv_enc from medicaid.dbo.enc_header_19 union
	select derv_enc from medicaid.dbo.enc_header_20 union
	select derv_enc from medicaid.dbo.enc_header_21) z;
	
```

## 1.9 Get total distinct ICNs from proc tables from SPC server
```{sql, connection = spc, output.var = total_distinct_ICN_proc_spc}
...
colnames= c("dw_head", "mcd_head_TACC", "mcd_proc_TACC",
            "mcd_joint_TACC", "mcd_head_SPC", "mcd_proc_SPC",
            "mcd_joint_SPC")
...
count_of_distinct_claims_summary = data.frame(cbind(
  distinct_ICN_header_dwstage$count,
  distinct_ICN_header_TACCmcd$count,
  distinct_ICN_proc_TACCmcd$count,
  distinct_ICN_innerjoin_TACCmcd$count))

#correct totals from TACC
count_of_distinct_claims_summary[11,2] = total_fix_temp[1,1]
count_of_distinct_claims_summary[11,3] = total_fix_temp[2,1]
count_of_distinct_claims_summary[11,4] = total_fix_temp[3,1]
...
count_of_distinct_claims_summary[11,5] = total_distinct_ICN_header_spc[1,1]
...
TACCvSPC = count_of_distinct_claims_summary %>%
  mutate (head_diff = mcd_head_SPC - mcd_head_TACC ) %>%
...
DWvMCD = count_of_distinct_claims_summary %>%
  mutate (diff = mcd_joint_TACC - dw_head ) %>%
  select (1, 4, 8)
...
yearly_mismatch = count_of_distinct_claims_summary %>%
  mutate (TACCheader_excess =  mcd_head_TACC - mcd_joint_TACC ) %>%
  mutate (TACCproc_excess =  mcd_proc_TACC - mcd_joint_TACC ) %>%
  mutate (SPCheader_excess =  mcd_head_SPC - mcd_joint_SPC ) %>%
...
description1 <- c("Count of distinct claims in dw_staging.claim_header",
                  "Count of distinct claims in medicaid.clm_header and medicaid.enc_header (TACC)",
                  "Count of distinct claims in medicaid.clm_proc and medicaid.enc_proc (TACC)",
                  "Count of distinct claims in headers inner join to procs (TACC)",
                  "Count of distinct claims from raw data in SPC server (headers)",
                  "Count of distinct claims from raw data in SPC server (procs)",
                  "Count of distinct claims from raw data in SPC server (headers inner join to procs by year)")
table1_legend <- data.frame(column_name1, description1)
colnames(table1_legend) = legend_colnames

column_name2 = colnames(TACCvSPC)
description2 <- c("Count of distinct claims in medicaid.clm_header and medicaid.enc_header (TACC)",
                  "Count of distinct claims from raw data in SPC server (headers)",
                  "Difference of distinct claims between TACC medicaid schema and SPC raw data (headers)",
                  "Count of distinct claims in medicaid.clm_proc and medicaid.enc_proc (TACC)",
                  "Count of distinct claims from raw data in SPC server (procs)",
...
description3 <- c("Count of distinct claims in dw_staging.claim_header",
                  "Count of distinct claims in medicaid.clm_header and medicaid.enc_header (TACC)",
...
description4 <- c("Count of distinct claims in medicaid.clm_header and medicaid.enc_header (TACC)",
                  "Count of distinct claims in medicaid.clm_proc and medicaid.enc_proc (TACC)",
                  "Count of distinct claims in headers inner join to procs (TACC)",
                  "Count of distinct claims in headers that are not in the joined table (TACC)",
                  "Count of distinct claims in procs that are not in the joined table (TACC)",
                  "Count of distinct claims from raw data in SPC server (headers)",
                  "Count of distinct claims from raw data in SPC server (procs)",
                  "Count of distinct claims from raw data in SPC server (headers inner join to procs by year)",
                  "Count of distinct claims in headers that are not in the joined table (SPC)",
...
#SECTION 98: Make a table of all ICNs and their table of origin
Ended up not using this, because the counts aren't too off
```{sql, connection = spc, output.var = table_list_clm, eval=FALSE}

--get all the table names (claims)

select TABLE_NAME
from information_schema.tables
where	table_name like 'clm_detail_%' or
	table_name like 'clm_dx_%' or
	table_name like 'clm_header_%' or
	table_name like 'clm_proc_%'
order by table_name;

```

```{sql, connection = spc, output.var = table_list_enc, eval=FALSE}

--get all the table names (encounters)

select TABLE_NAME
from information_schema.tables
where table_name like 'enc_det%' or
	table_name like 'enc_dx_%' or
	table_name like 'enc_header_%' or
	table_name like 'enc_proc_%'
order by table_name;

```

```{r, eval=FALSE}
  sql_geticns_clm = 'insert into work.dbo.xz_temp3_clm
  select icn, \'@tablename\' as table_name from medicaid.dbo.@tablename;'

  sql_geticns_enc = 'insert into work.dbo.xz_temp3_enc
  select derv_enc, \'@tablename\' as table_name from medicaid.dbo.@tablename;'
```
...
dbGetQuery(spc, 'select icn, \'CLM_DETAIL_12\' as table_name into work.dbo.xz_temp3_clm from medicaid.dbo.CLM_DETAIL_12;')
dbGetQuery(spc, 'select derv_enc, \'ENC_DET_13\' as table_name into work.dbo.xz_temp3_enc from medicaid.dbo.ENC_DET_13;')

```

```{sql, connection = spc, eval=FALSE}
--The tablenames are auto-assigned small data types, so we alter the table

ALTER TABLE work.dbo.xz_temp3_clm
ALTER COLUMN table_name varchar(50);

ALTER TABLE work.dbo.xz_temp3_enc
ALTER COLUMN table_name varchar(50);
...
#had trouble with and skipped CLM_DETAIL_1819_HTW, CLM_DETAIL_CLIENT_TYPE_21
for (i in 21:nrow(table_list_clm)){
  
  print(paste("Getting icns for table ", table_list_clm[i,1]))
  
  sql_geticns_clm.mod = sql_geticns_clm %>% gsub(pattern = '@tablename', replacement = table_list_clm[i,1])
  dbGetQuery(spc, sql_geticns_clm.mod)
...
for (i in 2:nrow(table_list_enc)){
  
  print(paste("Getting icns for table ", table_list_enc[i,1]))
  
  sql_geticns_enc.mod = sql_geticns_enc %>% gsub(pattern = '@tablename', replacement = table_list_enc[i,1])
  dbGetQuery(spc, sql_geticns_enc.mod)
