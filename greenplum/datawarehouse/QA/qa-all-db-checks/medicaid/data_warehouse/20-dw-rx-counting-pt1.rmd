...
#Purpose:
This R markdown script counts distinct pharmacy rx for medicaid from SPCDEDPWPVS1 only
...
#run for each year
sql1 = 'insert into work.dbo.xz_dwqa_chiprx
select \'20@yr\' as fy, concat(pcn, ndc, replace(rx_fill_dt, \'-\', \'\')) as rx_id
from medicaid.dbo.chip_rx_fy@yr
where pcn != \'000000000\';

insert into work.dbo.xz_dwqa_ffsrx
select \'20@yr\' as fy, concat(pcn, ndc, replace(rx_fill_dt, \'-\', \'\')) as rx_id
from medicaid.dbo.ffs_rx_fy@yr
where pcn != \'000000000\';

insert into work.dbo.xz_dwqa_mcorx
select \'20@yr\' as fy, concat(pcn, ndc, replace(rx_fill_dt, \'-\', \'\')) as rx_id
from medicaid.dbo.mco_rx_fy@yr
where pcn != \'000000000\';'
...
```{r}
dbGetQuery(spc, 'drop table if exists work.dbo.xz_dwqa_chiprx;')
dbGetQuery(spc, 'drop table if exists work.dbo.xz_dwqa_ffsrx;')
dbGetQuery(spc, 'drop table if exists work.dbo.xz_dwqa_mcorx;')
...
drop table if exists work.dbo.xz_dwqa_rxtemp2;
...
select * from work.dbo.xz_dwqa_rxtemp2 order by fy;
