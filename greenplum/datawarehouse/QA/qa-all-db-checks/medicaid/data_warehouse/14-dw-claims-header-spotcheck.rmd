---
title: "dw-claims-header-spotcheck"
author: "XRZ"
date: '2022-11-30'
...
# PURPOSE
This R markdown script spot checks for matching header information in data_warehouse
...
alter column claim_type varchar(1) NULL;'
...
for (yr in starting_yr:ending_yr){
  ...
  if (yr == 12){
    ...
    tic("sample from claims")
    dbExecute(spc, spc_sql1.mod)
    toc()
    
    # FIX: claim_type initializes as not allowing NULL from CLM table but
    # sometimes ENC tables have nulls so we need to allow nulls
...
select a.*, b.from_date_of_service, b.claim_type as dw_claim_type, b.total_charge_amount, b.total_allowed_amount,
	...
	case when b.claim_type is null and a.claim_type is not null then 1
	   when b.claim_type::text != trim(a.claim_type)::text then 1 else 0 end as claim_type_mismatch,
	...
	case when b.provider_type is null then 1 when trim(b.provider_type)::text != trim(a.provider_type)::text then 1 else 0 end as provider_type_mismatch
into dev.xz_dwqa_temp2
from dev.xz_dwqa_temp1 a left join data_warehouse.claim_header_1_prt_mdcd b on a.icn = b.claim_id_src;
...
writeData(wb, sheet = 1, "Mismatches found in claim_header tables between SPC and data_warehouse", startCol=1, startRow=1)
