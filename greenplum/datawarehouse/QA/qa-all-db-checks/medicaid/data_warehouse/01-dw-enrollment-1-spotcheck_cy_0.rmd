...
spc_sql1 = 'insert into work.dbo.xz_mcd_reconciliation_cy_etl
...
spc_sql2 = 'insert into work.dbo.xz_mcd_reconciliation_cy_etl
...
spc_sql3 = 'insert into work.dbo.xz_mcd_reconciliation_cy_etl
select a.CLIENT_NBR, substring(a.ELIG_DATE, 1, 4) as CY, a.elig_date, try_cast(a.DOB as date) as dob, 
a.SEX, b.MCO_PROGRAM_NM as MCO, a.RACE, a.ZIP, a.SMIB,
case when a.me_code = \'W\' then 1 else 0 end as HTW
...
```{sql, connection = spc}
--clean dob
drop table if exists work.dbo.xz_mcd_reconciliation_cy_dob_t;
select client_nbr, dob, count(distinct elig_date) as count, max(elig_date) as recent
into work.dbo.xz_mcd_reconciliation_cy_dob_t
from work.dbo.xz_mcd_reconciliation_cy_etl
group by client_nbr, dob;

drop table if exists work.dbo.xz_mcd_reconciliation_cy_dob;
with cte as (select *,
  	    row_number() over (partition by client_nbr order by count desc, recent desc) as row_num
	from work.dbo.xz_mcd_reconciliation_cy_dob_t)

select client_nbr, dob
into work.dbo.xz_mcd_reconciliation_cy_dob
from cte
where row_num = 1;
...
--clean MCO
drop table if exists work.dbo.xz_mcd_reconciliation_cy_mco_t;
select a.client_nbr, a.cy, a.mco, b.priority, count(distinct a.elig_date) as count, max(a.elig_date) as recent
into work.dbo.xz_mcd_reconciliation_cy_mco_t
from work.dbo.xz_mcd_reconciliation_cy_etl a left join work.dbo.xz_mcd_mco_priority b on a.mco = b.plan_type
group by a.client_nbr, a.cy, a.mco, b.priority;

drop table if exists work.dbo.xz_mcd_reconciliation_cy_mco;
with cte as (select *,
  	    row_number() over (partition by client_nbr, cy order by count desc, priority, recent desc) as row_num
	from work.dbo.xz_mcd_reconciliation_cy_mco_t)

select client_nbr, cy, mco
into work.dbo.xz_mcd_reconciliation_cy_mco
from cte
where row_num = 1;
...
spc_sql4 = 'drop table if exists work.dbo.xz_mcd_reconciliation_cy_@var_t;
            drop table if exists work.dbo.xz_mcd_reconciliation_cy_@var;'

spc_sql5 = 'select client_nbr, cy, @var, count(distinct elig_date) as count, max(elig_date) as recent
into work.dbo.xz_mcd_reconciliation_cy_@var_t
from work.dbo.xz_mcd_reconciliation_cy_etl
group by client_nbr, cy, @var;

with cte as (select *,
  	    row_number() over (partition by client_nbr, cy order by count desc, recent desc) as row_num
	from work.dbo.xz_mcd_reconciliation_cy_@var_t)

select client_nbr, cy, @var
into work.dbo.xz_mcd_reconciliation_cy_@var
from cte
where row_num = 1;'

spc_sql6 = 'drop table if exists work.dbo.xz_mcd_reconcilation_cy_@var_t;'
```
...
into work.dbo.xz_mcd_enrl_cy_reconciled_t1
from  work.dbo.xz_mcd_enrl_cy_reconciled_t a
