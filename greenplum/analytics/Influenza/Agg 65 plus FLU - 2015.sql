---Influenza Vacc Prevalence 65+ for 2015---

drop table dev.wc_temp_mdcd65_flu_2015

create table dev.wc_temp_mdcd65_flu_2015 ( client_num text, fst_elig text, lst_elig text, zip3 char(3), sex char(1), age text, age_group text, vacc_flag text);

update dev.wc_temp_mdcd65_flu_2015 set vacc_flag = '0' where vacc_flag = '';

select count(*) , sum (vacc_flag::int) from dev.wc_temp_mdcd65_flu_2015



---optum and truven cohorts from DW
drop table dev.wc_flu_65plus_2015;

select uth_member_id, 
       a.gender_cd, 
       a.zip3,
       data_source 
 into dev.wc_flu_65plus_2015
...
insert into dev.wc_flu_65plus_2015
...
----get vacc rates
drop table dev.wc_flu_65plus_2015_vacc;

select distinct uth_member_id 
into dev.wc_flu_65plus_2015_vacc
...
where a.procedure_cd in ('90630','90654','90655','90655','90656','90657','90658','90658','90660','90661','90662',
      ...
      and a.year = 2015 
      and a.uth_member_id in ( select uth_member_id from dev.wc_flu_65plus_2015)
;


alter table dev.wc_flu_65plus_2015 add column vacc_flag int2 default 0;


update dev.wc_flu_65plus_2015 a set vacc_flag = 1
  from dev.wc_flu_65plus_2015_vacc b 
    where b.uth_member_id = a.uth_member_id
 ;


drop table dev.wc_flu_65plus_all_2015

select * 
into dev.wc_flu_65plus_all_2015
from ( 
select * from dev.wc_flu_65plus_2015
union 
select client_num::bigint, sex, zip3, 'mdcd' as data_source,  vacc_flag::int from dev.wc_temp_mdcd65_flu_2015
) inr 


select count(*) from dev.wc_flu_65plus_all_2015 
...
from dev.wc_flu_65plus_all_2015 
...
---all
	r_num := 0;
	r_den := (	select count(*) from dev.wc_flu_65plus_all_2015 );
...
---female 
	r_num := 0;
	r_den := (	select count(*) from dev.wc_flu_65plus_all_2015 where gender_cd = 'F' );
...
---male 
	r_num := 0;
	r_den := (	select count(*) from dev.wc_flu_65plus_all_2015 where gender_cd = 'M' );
...
select distinct gender_cd , data_source from dev.wc_flu_65plus_all_2015
...
from dev.wc_flu_65plus_all_2015 a 
group by data_source
...
---optum
--missing zip 753 772 
insert into dev.wc_flu_65plus_all_2015 values 
(0001,'M',753,'optz',0),
(0006,'F',753,'optz',0),
(0007,'M',772,'optz',0),
(0012,'F',772,'optz',0)
