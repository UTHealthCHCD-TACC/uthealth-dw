---Influenza Vacc Prevalence 65+ for 2016---

drop table dev.wc_temp_mdcd65_flu_2016

create table dev.wc_temp_mdcd65_flu_2016 ( client_num text, fst_elig text, lst_elig text, zip3 char(3), sex char(1), age text, age_group text, vacc_flag text);

update dev.wc_temp_mdcd65_flu_2016 set vacc_flag = '0' where vacc_flag = '';

select count(*) , sum (vacc_flag::int) from dev.wc_temp_mdcd65_flu_2016
...
insert into dev.wc_flu_65plus_2016
...
----get vacc rates
drop table dev.wc_flu_65plus_2016_vacc;
...
union 
select client_num::bigint, sex, zip3, 'mdcd' as data_source,  vacc_flag::int from dev.wc_temp_mdcd65_flu_2016
) inr 


select count(*) from dev.wc_flu_65plus_all_2016 
...
---all
	r_num := 0;
	r_den := (	select count(*) from dev.wc_flu_65plus_all_2016 );
...
---female 
	r_num := 0;
	r_den := (	select count(*) from dev.wc_flu_65plus_all_2016 where gender_cd = 'F' );
...
---male 
	r_num := 0;
	r_den := (	select count(*) from dev.wc_flu_65plus_all_2016 where gender_cd = 'M' );
...
---optum
--missing zip 753 772 
insert into dev.wc_flu_65plus_all_2016 values 
(0001,'M',753,'optz',0),
(0006,'F',753,'optz',0),
(0007,'M',772,'optz',0),
(0012,'F',772,'optz',0)
