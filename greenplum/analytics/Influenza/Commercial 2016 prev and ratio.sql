...
drop table dev.wc_flu_com_2016_vacc;
...
--prevalance all - row 51  -- truven weight 0.603
...
--prevalance all - row 51  -- optum weight 0.397
select ( sum(vacc_flag) / count(uth_member_id)::float )as prev, count(uth_member_id), sum(vacc_flag)
from dev.wc_flu_com_2016 a 
...
-- truven weight 0.603   optum weight 0.397 
...
order by a.zip3
 ...
 -- optum weight 0.397   missing zip 753 772 
select ( sum(vacc_flag) / count(uth_member_id)::float ) as prev, a.zip3 
from dev.wc_flu_com_2016 a 
where a.data_source = 'optz'
group by a.zip3 
