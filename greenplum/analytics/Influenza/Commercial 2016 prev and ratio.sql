...
drop table dev.wc_flu_com_2016;
...
delete from dev.wc_flu_com_2016 where age_group is null;

delete from dev.wc_flu_com_2016 where length(zip3::text) = 2

select distinct uth_member_id 
into dev.wc_flu_com_2016_vacc
...
where a.procedure_cd in ('90630','90654','90655','90655','90656','90657','90658','90658','90660','90661','90662',
      ...
      and a.year = 2016 
      and a.uth_member_id in ( select uth_member_id from dev.wc_flu_com_2016)
;



alter table dev.wc_flu_com_2016 add column vacc_flag int2 default 0;


update dev.wc_flu_com_2016 a set vacc_flag = 1
  from dev.wc_flu_com_2016_vacc b 
...
---all
	r_num := 0;
	r_den := (	select count(*) from dev.wc_flu_com_2016 );

	for r_num , r_data_source
	  in 
	select count(*), data_source 
	from dev.wc_flu_com_2016
...
---female 
	r_num := 0;
	r_den := (	select count(*) from dev.wc_flu_com_2016 where gender_cd = 'F' );
...
---male 
	r_num := 0;
	r_den := (	select count(*) from dev.wc_flu_com_2016 where gender_cd = 'M' );
...
---age_group
	...
	loop
		r_ag := r_ag+1;
		r_num := 0;
		r_den := (	select count(*) from dev.wc_flu_com_2016 where age_group = r_ag );
...
order by a.zip3
 ...
 insert into dev.wc_flu_com_2016 values 
(0001, 753,1,'M','optz',0),
(0002, 753,2,'M','optz',0),
(0003, 753,3,'M','optz',0),
(0004, 753,4,'F','optz',0),
