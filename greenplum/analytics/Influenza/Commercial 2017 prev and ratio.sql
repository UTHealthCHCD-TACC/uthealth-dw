...
delete from dev.wc_flu_com_2017 where length(zip3::text) = 2
...
---all
	r_num := 0;
	r_den := (	select count(*) from dev.wc_flu_com_2017 );
...
---female 
	r_num := 0;
	r_den := (	select count(*) from dev.wc_flu_com_2017 where gender_cd = 'F' );
...
---male 
	r_num := 0;
	r_den := (	select count(*) from dev.wc_flu_com_2017 where gender_cd = 'M' );
...
---age_group
	...
	loop
		r_ag := r_ag+1;
		r_num := 0;
		r_den := (	select count(*) from dev.wc_flu_com_2017 where age_group = r_ag );
...
order by a.zip3
 ...
 insert into dev.wc_flu_com_2017 values 
(0001, 753,1,'M','optz',0),
(0002, 753,2,'M','optz',0),
(0003, 753,3,'M','optz',0),
(0004, 753,4,'F','optz',0),
