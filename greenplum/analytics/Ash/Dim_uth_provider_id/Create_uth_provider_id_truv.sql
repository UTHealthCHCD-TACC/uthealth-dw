...
--select count(*) from data_warehouse.dim_uth_provider_id where data_source = 'truv'
--2364650
...
insert into data_warehouse.provider (data_source, uth_provider_id, provider_id_src )
select distinct d.data_source, d.uth_provider_id , d.provider_id_src 
	from data_warehouse.dim_uth_provider_id d
	left join data_warehouse.provider p2 on p2.uth_provider_id = d.uth_provider_id 
	where d.data_source = 'truv'
		and p2.uth_provider_id is null;	
	
vacuum analyze data_warehouse.provider;

--select count(*) from data_warehouse.provider ap where ap.data_source = 'truv'
--2364650	
-----------------------------------------------------------------------------------------
...
select 'truv' as data_source, "year" as data_year, 
	...
	from truven.ccaeo t 
	inner join data_warehouse.dim_uth_provider_id d on d.provider_id_src = cast(t.provid as numeric(10,0))::text
	where t.provid is not null
		and d.data_source = 'truv';	
	
--ccaes
insert into data_warehouse.claim_detail_provider_src1 	
select 'truv' as data_source, "year" as data_year, 
		enrolid::text as member_id_src , msclmid::text as claim_id_src , 
		seqnum  as claim_sequence_number, svcdate as from_date_of_service,
		d.uth_provider_id as bill_provider	
...
--select * from data_warehouse.claim_detail_provider_src1	
...
from data_warehouse.claim_detail_provider_src1;

--select * from data_warehouse.claim_detail_provider_src2	order by data_year, member_id_src, claim_id_src , claim_sequence_number
------------------------------------------------------------------------------------------------------------	
--delete old data
...
------------------------------------------------------------------------------------------------------------		
select count(*) from data_warehouse.claim_detail_provider where data_source = 'truv'	
