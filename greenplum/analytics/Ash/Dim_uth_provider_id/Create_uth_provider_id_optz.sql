...
analyze data_warehouse.dim_uth_provider_id;
...
--select * from data_warehouse.dim_uth_provider_id
--9719111
-----------------------------------------------------------------------------------------
...
insert into data_warehouse.provider (data_source, uth_provider_id, provider_id_src, npi, taxonomy1, taxonomy2, provider_type, state)
	select distinct 'optz' as data_source, u.uth_provider_id, u.provider_id_src , pb.npi, p.taxonomy1 , p.taxonomy2 , 
		...
		from optum_zip.provider p , optum_zip.provider_bridge pb, data_warehouse.dim_uth_provider_id u
...
--select * from data_warehouse.provider ap 
-----------------------------------------------------------------------------------------	
-----------------------------------------------------------------------------------------
...
select src.data_source , data_year , member_id_src , claim_id_src , claim_sequence_number , from_date_of_service , 
	...
	left outer join data_warehouse.dim_uth_provider_id ut_bill on ut_bill.provider_id_src::bigint = src.bill_provider 
	left outer join data_warehouse.dim_uth_provider_id ut_refer on ut_refer.provider_id_src::bigint = src.ref_provider 
 	left outer join data_warehouse.dim_uth_provider_id ut_service on ut_service.provider_id_src::bigint = src.other_provider 
 	left outer join data_warehouse.dim_uth_provider_id ut_rendering on ut_rendering.provider_id_src::bigint = src.perf_rn_provider; 
