...
select distinct 'optd' data_source, p.prov_unique::text 
	...
	left join dev.am_dim_uth_provider_id d on d.provider_id_src = p.prov_unique::text and d.data_source = 'optd'
...
--select * from dev.am_dim_uth_provider_id where data_source = 'optd'
--9719111
-----------------------------------------------------------------------------------------
...
insert into dev.am_provider (data_source, uth_provider_id, provider_id_src, npi, taxonomy1, taxonomy2, provider_type, state)
	select distinct 'optd' as data_source, u.uth_provider_id, u.provider_id_src , pb.npi, p.taxonomy1 , p.taxonomy2 , 
		...
		from optum_dod.provider p , optum_dod.provider_bridge pb, dev.am_dim_uth_provider_id u
...
--select * from dev.am_provider ap where ap.data_source = 'optd'
-----------------------------------------------------------------------------------------	
-----------------------------------------------------------------------------------------
...
select * 
	into dev.am_dim_uth_provider_id_optd
...
select src.data_source , data_year , member_id_src , claim_id_src , claim_sequence_number , from_date_of_service , 
	...
	left outer join dev.am_dim_uth_provider_id_optd ut_bill on ut_bill.provider_id_src::bigint = src.bill_provider 
	left outer join dev.am_dim_uth_provider_id_optd ut_refer on ut_refer.provider_id_src::bigint = src.ref_provider 
 	left outer join dev.am_dim_uth_provider_id_optd ut_service on ut_service.provider_id_src::bigint = src.other_provider 
 	left outer join dev.am_dim_uth_provider_id_optd ut_rendering on ut_rendering.provider_id_src::bigint = src.perf_rn_provider; 
