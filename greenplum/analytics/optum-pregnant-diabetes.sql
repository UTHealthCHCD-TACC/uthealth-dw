...
--Pregnant
join optum_dod.diagnostic preg on m.patid=preg.patid
join optum_dod.lu_diagnosis preg_lu on preg.diag=preg_lu.diag_cd and preg_lu.diag_desc ilike '%pregnan%'
--Diabetes
join optum_dod.diagnostic diabetic on m.patid=diabetic.patid
join optum_dod.lu_diagnosis diabetic_lu on diabetic.diag=diabetic_lu.diag_cd and diabetic_lu.diag_desc ilike '%diabet%' and diabetic.year <= 2016
--Filter
where m.gdr_cd='F'
and preg.year = 2016;
...
join optum_dod.lu_diagnosis d_lu on d.diag=d_lu.diag_cd and d_lu.diag_desc ilike '%pregnan%'
...
from dev.female m 
join optum_dod.diagnostic d on m.patid=d.patid
join optum_dod.lu_diagnosis d_lu on d.diag=d_lu.diag_cd and d_lu.diag_desc ilike '%diabet%'
where d.year <= 2016
...
join optum_dod.facility_detail f on pd.clmid=f.clmid;
