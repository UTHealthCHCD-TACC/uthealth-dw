---Commercial Annual Preventive Exam (APE) - 2018 

---optum and truven cohorts from DW
drop table dev.wc_ape_com_2018;
...
delete from dev.wc_ape_com_2018 where age_group is null;

delete from dev.wc_ape_com_2018 where length(zip3::text) = 2

drop table dev.wc_ape_com_2018_vacc;

select distinct uth_member_id 
into dev.wc_ape_com_2018_vacc
from data_warehouse.claim_detail a
where a.cpt_hcpcs in ('99381','99382','99383','99384','99385','99386','99387',
						 '99391','99392','99393','99394','99395','99396','99397',
						 'S0610','S0612','S0615')
      and a.year = 2018 
      and a.uth_member_id in ( select uth_member_id from dev.wc_ape_com_2018)
;


insert into dev.wc_ape_com_2018_vacc
select distinct uth_member_id 
from data_warehouse.claim_diag 
where diag_cd in ('Z0000','Z0001','Z00110','Z00111','Z00121','Z00129','Z003','Z01411','Z01419',
				  'V700','V700','V7231','V705','V703','V7284','V7285') 
      and year = 2018 
      and uth_member_id in ( select uth_member_id from dev.wc_ape_com_2018)
      and uth_member_id not in ( select uth_member_id from dev.wc_ape_com_2018_vacc)
;


alter table dev.wc_ape_com_2018 add column vacc_flag int2 default 0;


update dev.wc_ape_com_2018 a set vacc_flag = 1
  from dev.wc_ape_com_2018_vacc b 
...
---all
	r_num := 0;
	r_den := (	select count(*) from dev.wc_ape_com_2018 );

	for r_num , r_data_source
	  in 
	select count(*), data_source 
	from dev.wc_ape_com_2018 
...
---female 
	r_num := 0;
	r_den := (	select count(*) from dev.wc_ape_com_2018 where gender_cd = 'F' );
...
---male 
	r_num := 0;
	r_den := (	select count(*) from dev.wc_ape_com_2018 where gender_cd = 'M' );
...
---age_group
	...
	loop
		r_ag := r_ag+1;
		r_num := 0;
		r_den := (	select count(*) from dev.wc_ape_com_2018 where age_group = r_ag );
...
delete from dev.wc_ape_com_2018 where zip3 = '772';

 -- optum weight  missing zip 753 772 
insert into dev.wc_ape_com_2018 values 
(0001, 753,1,'M','optz',0),
(0002, 753,2,'M','optz',0),
(0003, 753,3,'M','optz',0),
(0004, 753,4,'F','optz',0),
