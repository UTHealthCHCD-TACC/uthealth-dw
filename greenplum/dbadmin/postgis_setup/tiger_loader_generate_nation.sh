...
${PSQL} -c "CREATE TABLE tiger_data.state_all(LIKE tiger.state INCLUDING ALL) DISTRIBUTED REPLICATED; "
${SHP2PGSQL} -D -c -s 4269 -g the_geom   -W "latin1" tl_2017_us_state.dbf tiger_staging.state | ${PSQL}
${PSQL} -c "SELECT loader_load_staged_data(lower('state'), lower('state_all')); INSERT INTO tiger.state SELECT * FROM tiger_data.state_all;"
...
${PSQL} -c "CREATE TABLE tiger_data.county_all(LIKE tiger.county INCLUDING ALL) DISTRIBUTED REPLICATED; " 
${SHP2PGSQL} -D -c -s 4269 -g the_geom   -W "latin1" tl_2017_us_county.dbf tiger_staging.county | ${PSQL}
${PSQL} -c "ALTER TABLE tiger_staging.county RENAME geoid TO cntyidfp;  SELECT loader_load_staged_data(lower('county'), lower('county_all')); INSERT INTO county SELECT * FROM tiger_data.county_all;"
	...
	${PSQL} -c "CREATE TABLE tiger_data.county_all_lookup (LIKE tiger.county_lookup INCLUDING ALL) DISTRIBUTED REPLICATED;"
...
cd /gisdata/www2.census.gov/geo/tiger/TIGER2017/ZCTA5
rm -f ${TMPDIR}/*.*
${PSQL} -c "DROP SCHEMA IF EXISTS tiger_staging CASCADE;"
${PSQL} -c "CREATE SCHEMA tiger_staging;"
...
${SHP2PGSQL} -D -c -s 4269 -g the_geom   -W "latin1" tl_2017_us_zcta510.dbf tiger_staging.zcta510 | ${PSQL}
${PSQL} -c "ALTER TABLE tiger.zcta5 DROP CONSTRAINT IF EXISTS enforce_geotype_the_geom; CREATE TABLE tiger_data.zcta5_all(LIKE tiger.zcta5 INCLUDING ALL);"
...
${PSQL} -c "INSERT INTO tiger.zcta5 SELECT * FROM tiger_data.zcta5_all;"
