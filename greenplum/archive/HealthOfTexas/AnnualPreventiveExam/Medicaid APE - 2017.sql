/****** Medicaid Annual Prev Exam 2017 ******/
...
FROM MEDICAID.dbo.CHIP_UTH_SFY2017_Final
  where elig_month between 201701 and 201712
...
drop table if exists stage.dbo.wc_mdcd_ape_wmde_2017
 
 select distinct client_nbr 
 into stage.dbo.wc_mdcd_ape_wmde_2017
 from ( 
 select distinct client_nbr 
...
--texas women and duel elig
 delete from stage.dbo.wc_mdcd_ape_2017_temp where client_nbr in ( select client_nbr from stage.dbo.wc_mdcd_ape_wmde_2017 )
...
delete from stage.dbo.wc_mdcd_ape_2017 where sex = 'U';

select count(*), age_group from stage.dbo.wc_mdcd_ape_2017  group by age_group 
...
----proc and hcpc
---------------------------------------------------------------------------------------------------------
drop table stage.dbo.wc_mdcd_ape_clm_2017 ;

select distinct derv_enc 
into stage.dbo.wc_mdcd_ape_clm_2017
from ( 
  select derv_enc
  ...
  where proc_cd in ('99381','99382','99383','99384','99385','99386','99387',
						 ...
						 'S0610','S0612','S0615')
  and FDOS_DT between '2017-01-01' and '2017-12-31'
union 
  select derv_enc
...
union 
  ...
  where proc_cd in ('99381','99382','99383','99384','99385','99386','99387',
						 '99391','99392','99393','99394','99395','99396','99397',
						 'S0610','S0612','S0615')
  and  FROM_DOS between '2017-01-01' and '2017-12-31'
union 
  select ICN
...
into  stage.dbo.wc_mdcd_ape_dx_2017
  from medicaid.dbo.htw_clm_dx_17 d 
    join MEDICAID.dbo.CLM_HEADER_17 h 
      on h.ICN = d.ICN 
     and h.HDR_FRM_DOS between '2017-01-01' and '2017-12-31'
...
insert into  stage.dbo.wc_mdcd_ape_dx_2017
  ...
  from MEDICAID.dbo.enc_dx_17 d 
    join MEDICAID.dbo.ENC_HEADER_17 h 
     ...
     and h.FRM_DOS between '2017-01-01' and '2017-12-31'
   where (   d.DX_CD_1 in (select dx_cd from stage.dbo.wc_mdcd_ape_diag) 
        or d.DX_CD_2 in (select dx_cd from stage.dbo.wc_mdcd_ape_diag) 
        or d.DX_CD_3 in (select dx_cd from stage.dbo.wc_mdcd_ape_diag) 
        or d.DX_CD_4 in (select dx_cd from stage.dbo.wc_mdcd_ape_diag) 
...
--------------------------------------------------------------------------------------------------------
drop table stage.dbo.wc_mdcd_ape_mem_2017;

select distinct mem_id 
into stage.dbo.wc_mdcd_ape_mem_2017
from (
		select distinct a.mem_id 
		from medicaid.dbo.enc_proc_17 a 
		where a.DERV_ENC in (select derv_enc from stage.dbo.wc_mdcd_ape_clm_2017) 
	union 
		select distinct a.mem_id 
	...
	union 
		...
		where a.ICN in (select derv_enc from stage.dbo.wc_mdcd_ape_clm_2017) 
    union 
		select a.PCN
...
select * 
from  stage.dbo.wc_mdcd_ape_mem_2017

-------------------------------------------------

alter table stage.dbo.wc_mdcd_ape_2017 add vacc_flag int default 0;


update stage.dbo.wc_mdcd_ape_2017 set vacc_flag = 1
  from stage.dbo.wc_mdcd_ape_mem_2017 b 
...
delete from stage.dbo.wc_mdcd_ape_2017 where age_group = 7;

select count(*), count(distinct client_nbr), sum(vacc_flag) from stage.dbo.wc_mdcd_ape_2017;
