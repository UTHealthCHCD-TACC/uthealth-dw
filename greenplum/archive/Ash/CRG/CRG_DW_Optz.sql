drop table if exists dev.am_crg_dw_optz_members;
drop table if exists dev.am_crg_dw_optz_claim_detail;
drop table if exists dev.am_crg_dw_optz_claim_diag_distinct;
drop table if exists dev.am_crg_dw_optz_claim_primary_diag;
drop table if exists dev.am_crg_dw_optz_claim_secondary_diag;
drop table if exists dev.am_crg_dw_optz_claim_procs_distinct;
drop table if exists dev.am_crg_dw_optz_claim_external_cause_diagnosis;
drop table if exists dev.am_crg_dw_optz_claim_procs;
drop table if exists dev.am_crg_dw_optz_claim_cpt_hcpcs;
drop table if exists dev.am_crg_dw_optz_claims;
drop table if exists dev.am_crg_dw_optz_final;
------------------------------------------------------------------------------
--get members
drop table if exists dev.am_crg_dw_optz_members;
	select distinct data_source, 
			...
			mem.gender_cd, 
			dob_derived
		into dev.am_crg_dw_optz_members
...
--select count(*) from dev.am_crg_dw_optz_members
------------------------------------------------------------------------------			
--get claim detail
drop table if exists dev.am_crg_dw_optz_claim_detail;
	select distinct cd.uth_claim_id ,
			...
			cd.revenue_cd 
		into dev.am_crg_dw_optz_claim_detail
		from data_warehouse.claim_detail cd , dev.am_crg_dw_optz_members m 
		where cd.uth_member_id = m.uth_member_id 
			and cd.data_source = 'optz'			
			and cd.year = 2019
		order by cd.uth_claim_id, cd.from_date_of_service;

--select * from dev.am_crg_dw_optz_claim_detail d order by uth_claim_id, claim_sequence_number
------------------------------------------------------------------------------
...
select distinct cd.data_source,cd.year, cd.uth_member_id, cd.uth_claim_id, cd.claim_sequence_number, cd.from_date_of_service , diag_cd, diag_position, icd_type, poa_src 
	into dev.am_crg_dw_optz_claim_diag_distinct
	from data_warehouse.claim_diag cd , dev.am_crg_dw_optz_claim_detail d
	where cd.uth_claim_id = d.uth_claim_id
		and cd.uth_member_id = d.uth_member_id
		and cd.data_source = 'optz'			
		and cd.year = 2019;
	
--select * from dev.am_crg_dw_optz_claim_diag_distinct	
------------------------------------------------------------------------------	
drop table if exists dev.am_crg_dw_optz_claim_primary_diag;
	
	select distinct uth_claim_id , diag_cd , icd_type 		
		into dev.am_crg_dw_optz_claim_primary_diag
		from dev.am_crg_dw_optz_claim_diag_distinct 
		where diag_position = 1
			and claim_sequence_number = 1;		
 
--select * from dev.am_crg_dw_optz_claim_primary_diag
------------------------------------------------------------------------------
...
with cte as (
				select d.uth_claim_id , 					
					...
					from dev.am_crg_dw_optz_claim_diag_distinct d
					where d.diag_position != 1						
					group by d.uth_claim_id
				)				
	select distinct uth_claim_id , diags		
		into dev.am_crg_dw_optz_claim_secondary_diag
		from cte;

--select * from dev.am_crg_dw_optz_claim_secondary_diag			
------------------------------------------------------------------------------
...
select distinct uth_claim_id , diags		
		into dev.am_crg_dw_optz_claim_external_cause_diagnosis
		from ctex;
...
select distinct data_source, "year", p.uth_claim_id, p.uth_member_id, p.claim_sequence_number, p.from_date_of_service , proc_cd, proc_position, icd_type
	into dev.am_crg_dw_optz_claim_procs_distinct
	from data_warehouse.claim_icd_proc p , dev.am_crg_dw_optz_claim_detail cd
	where p.uth_claim_id = cd.uth_claim_id 
		and p.uth_member_id = cd.uth_member_id 
		and p.data_source = 'optz'			
		and p.year = 2019;

--select * from dev.am_crg_dw_optz_claim_procs_distinct;	
------------------------------------------------------------------------------			
...
with cte2 as (
				select p.uth_claim_id , 						
					...
					from dev.am_crg_dw_optz_claim_procs_distinct p					
					group by p.uth_claim_id 
				)				
	select distinct uth_claim_id, 			
			procs,
			procedure_date
		into dev.am_crg_dw_optz_claim_procs
		from cte2;

--select * from dev.am_crg_dw_optz_claim_procs	
------------------------------------------------------------------------------	
--concate cpt_hcpcs
drop table if exists dev.am_crg_dw_optz_claim_cpt_hcpcs;					
with cte3 as (
				select cd.uth_claim_id , 						
					...
					from dev.am_crg_dw_optz_claim_detail cd	
					where cd.cpt_hcpcs is not null
					group by cd.uth_claim_id 
				)				
	select distinct uth_claim_id, 			
			cpt_hcpcs			
		into dev.am_crg_dw_optz_claim_cpt_hcpcs
		from cte3 	;

--select * from dev.am_crg_dw_optz_claim_cpt_hcpcs
------------------------------------------------------------------------------		
--get distinct claimids
drop table if exists dev.am_crg_dw_optz_claims;
	select distinct d.uth_claim_id , 		
			max(claim_sequence_number ) as idx
		into dev.am_crg_dw_optz_claims
		from dev.am_crg_dw_optz_claim_detail d
		group by d.uth_claim_id ;

--select * from dev.am_crg_dw_optz_claims	
------------------------------------------------------------------------------		
--create final result table	
drop table if exists dev.am_crg_dw_optz_final;
	select distinct 
			c.member_id_src as PatientId, 																--1			
			dev.fn_get_crg_valid_cd('optz', 'sex', m.gender_cd) as Sex, 								--2
			...
			case when c.discharge_date is not null then dev.fn_get_crg_valid_cd('optz', 'dischargestatus', c.discharge_status)
				else null 
			end as DischargeStatus,																		--9	
			c.type_of_bill as TypeOfBill,																--10
			dev.fn_get_crg_valid_cd('optz', 'placeofservice', c.place_of_service) as PlaceOfService,	--11
			c.provider_type as ProviderType,															--12
			dev.fn_get_crg_valid_cd('optz', 'icdversionqualifier', pd.icd_type) as ICDVersionQualifier,	--13
			...
			ed.diags as ExternalCauseOfInjuryDiagnosis,														--17
	 		...
	 		to_char(p.procedure_date ,'MMDDYYYY') as "Procedure	Date",									--20
	 	 	...
	 	 	dev.fn_get_crg_valid_cd('optz', 'placeofservice', c.place_of_service) as ItemPlaceOfService,--23 
			...
			dev.fn_get_crg_valid_cd('optz', 'itemsiteofservice', c.place_of_service) as ItemSiteOfService,	--35
			dev.fn_get_crg_valid_cd('optz', 'siteofservice', c.place_of_service) as SiteOfService		--36
			--'\n' as "NewLine"																			--37
		into dev.am_crg_dw_optz_final																	
		from dev.am_crg_dw_optz_members m 	 
		inner join dev.am_crg_dw_optz_claim_detail c on m.uth_member_id = c.uth_member_id	
		inner join dev.am_crg_dw_optz_claims cl on cl.uth_claim_id = c.uth_claim_id 
		inner join dev.am_crg_dw_optz_claim_primary_diag pd on pd.uth_claim_id = c.uth_claim_id 
		left join dev.am_crg_dw_optz_claim_secondary_diag sd on sd.uth_claim_id = c.uth_claim_id 
		left join dev.am_crg_dw_optz_claim_procs p on p.uth_claim_id = c.uth_claim_id 
		left join dev.am_crg_dw_optz_claim_cpt_hcpcs h on h.uth_claim_id = c.uth_claim_id 
		left join dev.am_crg_dw_optz_claim_external_cause_diagnosis ed on ed.uth_claim_id = c.uth_claim_id 
		where c.uth_claim_id is not null	
			and cl.idx = c.claim_sequence_number		
		order by 1, 7 ;
	
---------------------------------------------------------------------------------------------------------------------	
--select distinct * from dev.am_crg_dw_optz_final order by 1, 7 ;
