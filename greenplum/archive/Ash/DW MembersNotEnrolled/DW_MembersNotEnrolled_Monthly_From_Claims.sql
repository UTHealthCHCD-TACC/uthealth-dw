...
as
select c.data_source , c.year as data_year, c.uth_member_id , c.member_id_src , date_part('month', c.from_date_of_service) as claim_month,
...
create table dev.am_members_not_enrolled_monthly 
with(appendonly=true,orientation=column)
as
...
create table dev.am_member_enrollment_monthly 
with(appendonly=true,orientation=column)
as
select * 
...
--ALTER TABLE dev.am_member_enrollment_monthly rename column year to data_year;
...
insert into dev.am_member_enrollment_monthly(data_source, year, month_year_id , uth_member_id , gender_cd , dob_derived , death_date , 
											 claim_created_flag , rx_coverage , fiscal_year, age_derived )
...
create table dev.am_monthly_enrolled_members_by_claims 
with(appendonly=true,orientation=column)
as
