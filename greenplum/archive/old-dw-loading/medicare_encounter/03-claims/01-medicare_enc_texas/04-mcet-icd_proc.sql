 ...
 **************************/
--initialize table
drop table if exists dw_staging.mcet_proc_etl;

create table dw_staging.mcet_proc_etl as
select enc_join_key, bene_id, prcdr_dt1::date as prcdr_dt, 0 as pos, icd_prcdr_cd1 as proc_cd,
...
declare 
	...
	i int;
	table_names text[]:= array['ip', 'snf', 'op'];
	table_name text;
begin
...
vacuum analyze dw_staging.mcet_proc_etl;
--select * from dw_staging.mcet_proc_etl;
--select count(*) from dw_staging.mcet_proc_etl;
--9096292
...
drop table if exists dw_staging.mcet_claim_icd_proc;

create table dw_staging.mcet_claim_icd_proc 
...
insert into dw_staging.mcet_claim_icd_proc(
...
from dw_staging.mcet_proc_etl a
...
vacuum analyze dw_staging.mcet_claim_icd_proc;

--select * from dw_staging.mcet_claim_icd_proc;

--drop temp table
drop table if exists dw_staging.mcet_proc_etl;

select 'mcet claim icd proc script finished at ' || current_timestamp as message;
