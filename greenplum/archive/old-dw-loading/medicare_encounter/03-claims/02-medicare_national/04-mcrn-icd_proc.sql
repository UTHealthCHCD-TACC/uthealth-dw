 ...
 **************************/
--initialize table
drop table if exists dw_staging.mcrn_proc_etl;

create table dw_staging.mcrn_proc_etl as
...
--loops through each table
	for table_name in select unnest(table_names) loop
		...
		for i in 1..25 loop
			...
			execute 'insert into dw_staging.mcrn_proc_etl
...
vacuum analyze dw_staging.mcrn_proc_etl;
--select * from dw_staging.mcrn_proc_etl;
--select count(*) from dw_staging.mcrn_proc_etl;
--9096292
...
drop table if exists dw_staging.mcrn_claim_icd_proc;

create table dw_staging.mcrn_claim_icd_proc 
...
insert into dw_staging.mcrn_claim_icd_proc(
